...
public class CommanWSHumanTaskHandler implements WorkItemHandler {

	private static Logger logger = LoggerFactory
			.getLogger(CommanWSHumanTaskHandler.class);
	...
	public List<JbpmVo> jbpms = Lists.newArrayList();

	public CommanWSHumanTaskHandler(KnowledgeRuntime session) {
	...
	public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
		...
		boolean flag = true;
		for (JbpmVo jbpm : jbpms) {
			if (workItem.getId() == jbpm.getWorkitemId()) {
				flag = false;
				break;
			}
		}
		if (flag) {
			...
			jbpmVo.setNodeName((String) workItem.getParameter("TaskName"));
			jbpmVo.setActorId((String) workItem.getParameter("ActorId"));
			jbpmVo.setSessionId(sessionId);
			jbpmVo.setWorkitemId(workItem.getId());
			jbpmVo.setProcessId(workItem.getProcessInstanceId());
			jbpmVo.setGroupId((String) workItem.getParameter("GroupId"));

			try {
				if (null != workItem.getParameter("Content")) {
					String json = (String) workItem.getParameter("Content");
					if (StringUtils.isNotBlank(json)) {
						ObjectMapper mapper = new ObjectMapper();
					}
				}
			} catch (Exception e) {
				logger.error("task handle error.", e);
			}
	...
	private class GetCompletedTaskResponseHandler extends
		...
		public void execute(Task task) {
			...
			if (task.getTaskData().getStatus() == Status.Completed) {
				// System.out.println("Notification of completed task " +
				// workItemId);
				...
				if (contentId != -1) {
					...
					try {
						...
						throw new JbpmException(
								"workitem occurs error,please contact admin.");
					}
				}
			} else {
				// System.out.println("Notification of aborted task " +
				// workItemId);
	...
	private class GetResultContentResponseHandler extends
			AbstractBaseResponseHandler implements GetContentResponseHandler {
